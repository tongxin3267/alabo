...
namespace Alabo.App.Shop.Category.Domain.Services {
    ...
    public class CategoryService : ServiceBase<Entities.Category, Guid>, ICategoryService {
        ...
        public Entities.Category GetSingle(Guid guid) {
        ...
        public string AddOrUpdateOrDeleteProductCategoryData(Product.Domain.Entities.Product product,
            ...
            var productCategory = new Entities.Category(); // 需要保存的类目数据信息
        ...
        public Tuple<ServiceResult, Entities.Category> Delete(Guid id) {
            var result = ServiceResult.Success;
            if (id == Guid.Empty) {
                return Tuple.Create(ServiceResult.FailedWithMessage("删除失败"), new Entities.Category());
            }

            var model = Resolve<ICategoryService>().GetSingle(id);
            var context = Alabo.Helpers.Ioc.Resolve<IUserRepository>().RepositoryContext;
        ...
        public CategoryService(IUnitOfWork unitOfWork, IRepository<Entities.Category, Guid> repository) : base(unitOfWork, repository) {
        }
    }
}