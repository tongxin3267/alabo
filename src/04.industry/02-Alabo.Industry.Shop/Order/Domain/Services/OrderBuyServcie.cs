...
namespace Alabo.App.Shop.Order.Domain.Services {
    ...
    public class OrderBuyServcie : ServiceBase, IOrderBuyServcie {
        ...
        public Tuple<ServiceResult, BuyOutput> Buy(BuyInput orderBuyInput) {
            ...
            var cacheAddress = orderPriceCache.UserAddresses.FirstOrDefault(r => r.Id == orderBuyInput.AddressId.ToObjectId());
            ...
            var address = Resolve<IUserAddressService>().GetUserAddress(orderBuyInput.AddressId.ToObjectId(), orderBuyInput.UserId);
            if (address == null) {
            ...
            latestStoreProductSku.StoreItems.ForEach(store => {
                skuItems.AddRange(store.ProductSkuItems);
            });
            ...
            if (!orderBuyInput.PaymentAmount.EqualsDigits(
                ...
                latestStoreOrderPrice.FeeAmount, 2)) {
                return Tuple.Create(ServiceResult.FailedWithMessage("人民币支付金额计算错误"), new BuyOutput());
            }
            ...
            if (orderBuyInput.IsFromOrder) {
                var dynamicInstance =
                    Activator.CreateInstance("Alabo.App.Erp.Stock.Domain.Services.ProductBuyStockService".GetTypeByFullName());
            ...
            try {
                ...
                foreach (var storeOrderItem in orderBuyInput.StoreOrders) {
                    ...
                    if (couponList.Count > 0) {
                        var coupon = Resolve<IUserCouponService>().GetSingle(x => x.Id == couponList.FirstOrDefault().ToObjectId());
                    ...
                    var order = new Entities.Order {
                        ...
                        PayId = 0,
                        PaymentAmount = orderPaymentPrice.Item1 + orderFeeAmount - couponAmount, // 获取订单实际支付价格(订单支付价格+服务费）
                    };
                    // 订单扩展数据
                    ...
                    //更新限时购活动库存
                    timeLimitActivities.ForEach(item => {
                        Resolve<IActivityService>().UpdateNoTracking(item);
                    });
                ...
                Resolve<IOrderService>().Update(r => {
                    r.PayId = payResult.Item2.Id;
                }, e => orderIds.Contains(e.Id));
                ...
                foreach (var store in orderBuyInput.StoreOrders) {
                    ...
                    foreach (var sku in store.ProductSkuItems) {
                        Resolve<IProductSkuService>().Update(r => {
                            r.Stock = r.Stock - sku.Count;
                ...
                foreach (var item in productBuyCount) {
                    Resolve<IProductService>().Update(r => {
                        r.SoldCount = r.SoldCount + item.Value;
                    },
                        r => r.Id == item.Key);
                }
                ...
                // 如果使用了优惠券, set为已经使用
                if (couponAmount > 0) {
                    var userCoupon = Resolve<IUserCouponService>().GetSingle(x => x.UserId == orderBuyInput.UserId && x.Id == couponList.FirstOrDefault().ToObjectId());
                    if (userCoupon != null) {
        ...
        public Tuple<ServiceResult, StoreOrderPrice> GetPrice(UserOrderInput userOrderInput) {
            ...
            if (ObjectCache.TryGet(cacheKey, out OrderPriceCache orderPriceCache)) {
                var address = orderPriceCache.UserAddresses.FirstOrDefault(r => r.Id == userOrderInput.AddressId.ToObjectId());
                if (address == null) {
                    // 如果缓存不存在地址，则从数据库在读取一次，解决购物时，地址新增的情况
        ...
        public Tuple<ServiceResult, StoreOrderPrice> CountPrice(ref StoreProductSku storeProductSku,
            ...
            //会员价
            var productGradePrices = Resolve<IActivityApiService>().GetMemberDiscountPrice(storeProductSku.StoreItems, user);
            foreach (var storeItem in storeProductSku.StoreItems) {
                ...
                foreach (var productSkuItem in storeItem.ProductSkuItems) {
                    ...
                    var productGrade = productGradePrices.Find(p => p.ProductId == productSkuItem.ProductId && p.ProductSkuId == productSkuItem.ProductSkuId);
            ...
            if (userOrderInput != null) {
                        ...
                        if (orderMoneyItem != null) {
                            ...
                            if (!orderConfig.CashCanToOtherAssets) {
                                ...
                                if (moneyItemAccount < storeProductSku.TotalAmount) {
                                    result = ServiceResult.FailedWithMessage($"{orderMoneyItem.Name}余额不足，余额:{moneyItemAccount}");
                                    break;
                                }
                            }
                ...
                if (userOrderInput.CouponJson.IsNotNullOrEmpty()) {
                    ...
                    if (coupList.Count > 0) {
                        // 判断现金最小支付额度
                        var productAmount = storeOrderPrice.ProductAmount;
                        var coupon = Resolve<IUserCouponService>().GetSingle(x => x.Id == coupList.FirstOrDefault().ToObjectId());
                        if (coupon != null) {
        ...
        private StoreProductSku BuyInputToStoreProductSku(BuyInput buyInput) {
            ...
            var allProductSkus = new List<ProductSkuItem>();
            storeProductSku.StoreItems.ToList().ForEach(r => {
                allProductSkus.AddRange(r.ProductSkuItems);
            });
            //店铺使用的资产
        ...
        private Tuple<decimal, ServiceResult, ReduceAmount> GetOrderProductPaymentPrice(
            ...
            var skuPayment = Math.Round(storeOrderSku.Amount - reduceMoney / storeOrderSku.Count * moneyConfig.RateFee + feeAmount, 4);
            if (skuPayment < 0) {
        ...
        public Tuple<ServiceResult, BuyOutput> Pay(long orderId, long userId) {
            ...
            var ordersList = new List<Entities.Order> {
                order
            };
            ...
            BuyOutput orderBuy = new BuyOutput {
