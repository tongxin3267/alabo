...
namespace Alabo.Industry.Shop.Deliveries.Domain.Services
{
    /// <summary>
    /// </summary>
    public class ShopStoreService : ServiceBase<Store, long>, IShopStoreService
    {
        ...
        public ShopStoreService(IUnitOfWork unitOfWork, IRepository<Store, long> repository) : base(unitOfWork,
            repository)
        {
        ...
        public Store PlanformStore()
        {
            ...
            if (planformUser == null) return null;
        ...
        public ServiceResult UpdateExtensions(long stroeId, StoreExtension storeExtension)
        {
            ...
            if (store == null) return ServiceResult.FailedWithMessage("店铺不存在,更新失败");

            store.Extension = ObjectExtension.ToJson(storeExtension);
            if (Update(store)) return ServiceResult.Success;
        ...
        public ServiceResult AddOrUpdate(ViewStore store)
        {
            ...
            if (user == null) return ServiceResult.FailedWithMessage("所属用户不存在");

            if (user.Status != Status.Normal) return ServiceResult.FailedWithMessage("该用户状态不正常");
            ...
            if (!store.ParentUserName.IsNullOrEmpty())
            {
                ...
                if (parentUser == null) return ServiceResult.FailedWithMessage("推荐用户不存在");
            ...
            if (find.Id == 0)
            {
                if (!Add(find)) result = ServiceResult.FailedWithMessage("供应商添加失败");
            }
            else
            {
                if (!Update(find)) result = ServiceResult.FailedWithMessage("供应商更新失败");
            }
        ...
        public IEnumerable<StoreItem> GetStoreItemListFromCache()
        {
            if (!ObjectCache.TryGet(_storeItemListCacheKey, out List<StoreItem> result))
            {
                ...
                if (result != null) ObjectCache.Set(_storeItemListCacheKey, result);
            }
        ...
        public Tuple<ServiceResult, decimal> CountExpressFee(long storeId, ObjectId templId, UserAddress userAddress,
            decimal weight)
        {
            var expressFee = 0m; // 快递费用
            var express = Resolve<IDeliveryTemplateService>().GetSingle(t => t.StoreId == storeId && t.Id == templId);
            if (express == null) return Tuple.Create(ServiceResult.FailedWithMessage("运费模板未找到"), expressFee);
            //卖家承担运费的模版，运费为0
        ...
        public Tuple<ServiceResult, decimal> CountExpressFee(long storeId, UserAddress userAddress,
            IList<ProductSkuItem> productSkuItems)
        {
            ...
            productGroups.ForEach(product =>
            {
                ...
                var template =
                    deliveryTemplates.FirstOrDefault(d => d.Id.ToString() == tempDelivery?.DeliveryTemplateId);
                if (template == null || template.TemplateType == DeliveryTemplateType.SellerBearTheFreight) return;
        ...
        private decimal CountFeeByRegion(DeliveryTemplate express, long regionId, decimal value)
        {
            ...
            if (regionTemplateFee != null) return CountFeeByWeight(regionTemplateFee, value);
            return -1;
        }
