...
namespace Alabo.App.Shop.Coupons.Domain.Entities
{
    ...
    public class Coupon : AggregateMongodbRoot<Coupon>, IAutoTable<Coupon>
    {
        ...
        public PageResult<Coupon> PageTable(object query, AutoBaseModel autoModel)
        {
            var model = new PagedList<Coupon>();
            var couponList = Resolve<ICouponService>().GetPagedList(query);
            return ToPageResult(couponList);
        }
