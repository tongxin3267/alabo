...
namespace Alabo.App.Shop.Order.Domain.Services {
    ...
    public class OrderActionService : ServiceBase<OrderAction, long>, IOrderActionService {
        ...
        public void DeleteCartBuyOrder(BuyInput orderBuyInput) {
            ...
            //find
            var cars = Resolve<ICartService>().GetList(e => e.UserId == orderBuyInput.UserId && storeList.Contains(e.StoreId) && productSkus.Contains(e.ProductSkuId)).ToList();
            cars.ForEach(item => {
                item.Status = Domains.Enums.Status.Deleted;
        ...
        public OrderActionService(IUnitOfWork unitOfWork, IRepository<OrderAction, long> repository) : base(unitOfWork, repository) {
        }
    }
}