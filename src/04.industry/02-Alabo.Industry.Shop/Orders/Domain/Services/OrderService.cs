...
namespace Alabo.App.Shop.Order.Domain.Services {
    ...
    /// <summary>
    /// </summary>
    public class OrderService : ServiceBase<Entities.Order, long>, IOrderService {
        ...
        public OrderShowOutput GetSingle(long id, long UserdId) {
            ...
            orderShow.Methods = isAdmin ? allMethods.Where(r => r.AllowUser == 2).ToList() : allMethods.Where(r => r.AllowUser == 0).ToList();
        ...
        public OrderShowOutput GetSingleAdmin(long id, long UserdId) {
            ...
            if (regionId > 0) {
                ...
                order.OrderExtension.UserAddress.AddressDescription = address + order.OrderExtension.UserAddress.AddressDescription;
            }
            ...
            if (isAdmin) {
                ...
                if (order.OrderExtension.RefundInfo != null) {
                    ...
                    //取消通过和拒绝按钮
                    if (order.OrderExtension.RefundInfo.Process == RefundStatus.Closed || order.OrderExtension.RefundInfo.Process == RefundStatus.Sucess || order.OrderExtension.RefundInfo.Process == RefundStatus.WaitSaleRefuse) {
                        // 同意 拒绝  退款  都移除
                        ...
                        if (order.OrderStatus == OrderStatus.AfterSale) {
                            orderShow.Methods = allMethods.Where(s => !s.Type.Equal("AdminRefundInfo")).Where(s => !s.Type.Equal("AdminRefund")).ToList();
                        } else {
                            orderShow.Methods = allMethods.Where(s => !s.Type.Equal("AdminRefundInfo")).ToList();
                        }
                    }
                    //增加售后详情
                    var after = Resolve<IOrderService>().GetMethodByStatus(OrderStatus.AfterSale).Where(r => r.AllowUser == 2).FirstOrDefault(s => s.Type.Equal("UserRefundInfo"));
                    if (after != null) {
                ...
                if (order.OrderExtension.RefundInfo != null) {
                    //申请按钮 移除
                    orderShow.Methods = allMethods.Where(r => r.AllowUser == 0).Where(s => !s.Type.Equal("Refund")).Where(s => !s.Type.Equal("AfterSale")).ToList();
                    //增加售后详情
        ...
        public OrderShowOutput GetOrderSingle(long id, long UserdId) {
            ...
            var allMethods = Resolve<IOrderService>().GetMethodByStatus(order.OrderStatus).Where(r => r.AllowUser == 0).ToList();
            //申请过退款的不能再申请第二次
            if (order.OrderExtension.RefundInfo != null) {
                //申请按钮 移除
                orderShow.Methods = allMethods.Where(s => !s.Type.Equal("Refund")).Where(s => !s.Type.Equal("AfterSale")).Where(s => !s.Type.Equal("UserRefundInfo")).ToList();
                //增加售后详情这里有可能为空记得判断
                var after = Resolve<IOrderService>().GetMethodByStatus(OrderStatus.AfterSale).Where(r => r.AllowUser == 0).FirstOrDefault(s => s.Type.Equal("UserRefundInfo"));
                if (after != null) {
                    orderShow.Methods.Add(after);
                }
            } else {
        ...
        public OrderShowOutput Show(long id) {
            ...
            orderShow.Methods = Resolve<IOrderService>().GetMethodByStatus(order.OrderStatus).Where(r => r.AllowUser == 1).ToList();
            //orderShow.Methods = orderShow.Methods.Where(e => e.Method == e.Method.Replace("User", "")).ToList();
        ...
        public string GetExpressInfo(string expressId) {
            ...
            String url = $"http://v.juhe.cn/exp/index?key=1e2e7e4a7afef7f2cc1dbb3795fc3d33&com={model.ExpressNo}&no={model.ExpressNum}";
        ...
        public PagedList<OrderListOutput> GetPageList(OrderListInput orderInput) {
            var query = new ExpressionQuery<Entities.Order>();
            ...
            if (orderInput.StoreId > 0) {
                query.And(e => e.StoreId == orderInput.StoreId);
            }
            if (orderInput.DeliverUserId > 0) {
                // 发货记录
            ...
            } else if (orderInput.LoginUserId > 0) {
                //订单记录
            ...
            foreach (var item in orders) {
                ...
                if (item.OrderStatus == OrderStatus.WaitingBuyerPay) {
                    ...
                    var productNotOnline = Resolve<IProductService>().GetList(s => s.ProductStatus != ProductStatus.Online && proids.Contains(s.Id));
                    foreach (var product in productNotOnline) {
        ...
        public Tuple<ServiceResult, Entities.Order> Delete(long id) {
            var result = ServiceResult.Success;
            var model = Resolve<IOrderService>().GetSingle(u => u.Id == id);
            if (model == null) {
                return Tuple.Create(ServiceResult.FailedWithMessage("删除失败"), new Entities.Order());
            }
        ...
        public Entities.Order GetSingle(long orderId) {
        ...
        public Entities.Order GetSingleWithProducts(long orderId) {
        ...
        public OrderService(IUnitOfWork unitOfWork, IRepository<Entities.Order, long> repository) : base(unitOfWork, repository) {
        }
    }
}