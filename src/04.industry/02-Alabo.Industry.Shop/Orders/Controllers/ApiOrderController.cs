...
namespace Alabo.Industry.Shop.Orders.Controllers
{
    ...
    public class ApiOrderController : ApiBaseController<Order, long>
    {
        ...
        public ApiResult PayGoodsAmount([FromBody] PayGoodsAmountInput input)
        {
            ...
            if (result)
            {
                var store = Resolve<IStoreService>().GetSingle(u => u.Id == order.StoreId.ToObjectId());
        ...
        public ApiResult GetOrderList()
        {
            var result = Resolve<IAdminTableService>()
                .ToExcel("Alabo.App.Shop.Order.Domain.Dtos.OrderToExcel", "IOrderActionService", "GetOrdersToExcel",
                    QueryDictionary().ToJson());
            if (result.Item1.Succeeded)
            {
