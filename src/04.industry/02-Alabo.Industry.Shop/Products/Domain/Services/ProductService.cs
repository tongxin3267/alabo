...
namespace Alabo.Industry.Shop.Products.Domain.Services
{
    ...
    ///     ProductService
    /// </summary>
    public class ProductService : ServiceBase<Product, long>, IProductService
    {
        /// <summary>
        ///     constructor
        /// </summary>
        ...
        public ProductService(IUnitOfWork unitOfWork, IRepository<Product, long> repository)
        ...
        public List<Product> GetProductsByRelationId(long relationId)
        {
        ...
        public Product GetShow(long id, long userId)
        {
            //product
            ...
            // 处理商品不显示规格和属性的
            if (product.ProductExtensions.ProductCategory != null &&
                product.ProductExtensions.ProductCategory.SalePropertys != null)
            {
        ...
        {
            var productList = Resolve<IProductService>()
                .GetList(u => u.Id != productId && u.ProductStatus == ProductStatus.Online);
        ...
        public List<TimeLimitBuyItem> GetTimeLimitBuyList()
        {
            ...
            var allActivities = Resolve<IActivityService>()
                .GetList(a => a.Key == key && a.IsEnable).ToList();
