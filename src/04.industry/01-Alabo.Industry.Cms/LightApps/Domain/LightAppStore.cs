...
namespace Alabo.Industry.Cms.LightApps.Domain {
    ...
    public static class LightAppStore {
        ...
        /// </summary>
        /// <returns></returns>
        public static IEnumerable<dynamic> QueryList(string tableName, List<(string Field, Operator op, string Value)> listOp) {
            var filter = FilterParser(listOp);
        ...
        public static dynamic ValueParse(string value) {
            if (value.Length == 24 && ObjectId.TryParse(value, out ObjectId rsObjectId)) {
                return rsObjectId;
            }

            if (value.Contains('.') && Decimal.TryParse(value, out Decimal rsDecimal)) {
                return rsDecimal;
            }
            ...
            if (long.TryParse(value, out long rsLong)) {
                return rsLong;
            }

            if (bool.TryParse(value, out bool rsBool)) {
                return rsBool;
            }
        ...
        public static FilterDefinition<dynamic> FilterParser(List<(string Field, Operator op, string Value)> opList) {
            ...
            foreach (var item in opList) {
                dynamic value = ValueParse(item.Value);
