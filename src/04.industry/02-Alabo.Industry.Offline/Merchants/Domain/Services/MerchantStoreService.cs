...
namespace Alabo.Industry.Offline.Merchants.Domain.Services
{
    ...
    {
        public MerchantStoreService(IUnitOfWork unitOfWork, IRepository<MerchantStore, ObjectId> repository) : base(
        ...
        public Tuple<Merchant, MerchantStore> GetMerchantByMerchantStoreId(string merchantStoreId)
        {
            ...
            {
                var merchant = Resolve<IMerchantService>()
                    .GetSingle(m => m.Id == merchantStore.MerchantId.ToObjectId());
