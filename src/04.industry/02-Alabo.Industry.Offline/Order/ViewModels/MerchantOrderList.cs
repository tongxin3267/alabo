...
namespace Alabo.App.Offline.Order.ViewModels
{
    ...
    public class MerchantOrderList : UIBase, IAutoTable<MerchantOrderList>, IAutoList
    {
        ...
        public PageResult<AutoListItem> PageList(object query,AutoBaseModel autoModel)
        {
            ...
            var pageList = PagedList<MerchantOrderList>.Create(orders.Result, orders.RecordCount, orders.PageSize, orders.PageIndex);
            //Builder
        ...
        {
            var tenant = Tenants.TenantContext.CurrentTenant;
            ...
            var orders = Resolve<IMerchantOrderService>().GetPagedList<MerchantOrder>(dic.ToJson(), expressionQuery.BuildExpression());
            orders.ForEach(item =>
            {
            ...
            var pageList = PagedList<MerchantOrderList>.Create(result, orders.RecordCount, orders.PageSize, orders.PageIndex);
