...
namespace Alabo.App.Offline.Order.Domain.Services
{
    public class MerchantCartService : ServiceBase<MerchantCart, ObjectId>, IMerchantCartService
    {
        ...
        public ServiceResult AddCart(MerchantCartInput input)
        {
            ...
            var product = Resolve<IMerchantProductService>().GetSingle(r => r.Id == input.MerchantProductId.ToObjectId());//&& r.MerchantStoreId == input.MerchantStoreId
            if (product == null)
            {
        ...
        public Tuple<ServiceResult, MerchantCartOutput> GetCart(long userId, string merchantStoreId)
        {
            ...
            var viewCarts = Resolve<IMerchantCartService>().GetList(e => e.UserId == userId && e.Status == Status.Normal).ToList();
