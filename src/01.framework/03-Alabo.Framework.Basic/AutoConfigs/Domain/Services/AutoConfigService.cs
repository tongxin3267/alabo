...
namespace Alabo.Framework.Basic.AutoConfigs.Domain.Services {
    ...
    public class AutoConfigService : ServiceBase<AutoConfig, long>, IAutoConfigService {
        ...
        public AutoConfigService(IUnitOfWork unitOfWork, IRepository<AutoConfig, long> repository) : base(unitOfWork, repository) {
        }
        ...
        public Type GetTypeByName(string name) {
            var types = GetAllTypes();
            foreach (var item in types) {
                if (item.Name.Equals(name, StringComparison.OrdinalIgnoreCase) || item.FullName.Equals(name, StringComparison.OrdinalIgnoreCase)) {
                    return item;
                }
            }
