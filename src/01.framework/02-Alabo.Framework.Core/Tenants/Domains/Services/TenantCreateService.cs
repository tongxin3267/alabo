...
namespace Alabo.Framework.Core.Tenants.Domains.Services
{
    public class TenantCreateService : ServiceBase, ITenantCreateService
    {
        ...
        public ServiceResult InitTenantDefaultData(TenantInit tenantInit)
        {
            ...
            if (HttpWeb.Site == null) return ServiceResult.FailedWithMessage("站点信息初始化错误");
            //初始化默认数据
        ...
        {
            if (!TenantContext.IsTenant) return ServiceResult.FailedWithMessage("非租户模式不能创建租户");
        ...
        {
            if (tenant.IsNullOrEmpty()) return ServiceResult.FailedWithMessage("租户标识不能为空");
        ...
        public ServiceResult NoTenant(string tenant)
        {
            ...
            if (isExists) return ServiceResult.FailedWithMessage("租户数据库已存在");
            return ServiceResult.Success;
        }
        ...
        private ServiceResult Check(TenantInit tenantInit)
        {
            ...
            if (TenantContext.CurrentTenant == TenantContext.Master) return ServiceResult.FailedWithMessage("主库不能初始化");

            if (tenantToken == "master") return ServiceResult.FailedWithMessage("主库不能初始化");
            if (tenantToken.IsNullOrEmpty()) return ServiceResult.FailedWithMessage("租户头不能为空");
            if (tenantInit.Mobile.IsNullOrEmpty()) return ServiceResult.FailedWithMessage("手机号码不能为空");

            if (!Resolve<IOpenService>().CheckMobile(tenantInit.Mobile))
                return ServiceResult.FailedWithMessage("手机号码格式不正确");
            if (tenantToken != tenantInit.Tenant) return ServiceResult.FailedWithMessage("租户标识输入错误");

            var siteId = RuntimeContext.Current.WebsiteConfig.OpenApiSetting.Id;
            var token = Token(tenantInit.Tenant, siteId);
            if (token != tenantInit.Token) return ServiceResult.FailedWithMessage("Token加密不正确");
            return ServiceResult.Success;
        }
    }
}