...
namespace Alabo.Framework.Core.Valids.Services
{
    public class ValidService : ServiceBase, IValidService
    {
        ...
        {
            if (userName.IsNullOrEmpty()) return ServiceResult.FailedWithMessage("用户名不能为空");
        ...
        {
            if (userId <= 0) return ServiceResult.FailedWithMessage("用户Id不能小于0");
        ...
        {
            if (gradeId.IsGuidNullOrEmpty()) return ServiceResult.FailedWithMessage("等级ID不能为空");
        ...
        public ServiceResult VerifySafePassword()
        {
            if (HttpWeb.HttpContext.Request.Form.ContainsKey("PayPassword"))
            {
                ...
                if (safePassword.IsNullOrEmpty()) return ServiceResult.FailedWithMessage("请输入支付密码");
