...
namespace Alabo.Domains.Repositories.SqlServer {
    ...
    public sealed partial class SqlHelper {
        ...
        public void CreateDataBase() {
            ...
            var connToMaster = "Server=localhost;DataBase=master;UID=sa;PWD=123456";
            //创建数据库的Sql语句，将来由已经存在的数据库连接执行此命令
            //先判断数据库及数据表是否存在
            var createDbStr = "if NOT exists" +
                              "(" + "select * from master.dbo.sysdatabases where name " + "= '" + "SSPU" + "')" +
                              "Create database [SSPU]";

            //创建一个新的SqlCommand对象
            var cmd = new SqlCommand(createDbStr, conn);
            //首先连接到已经存在的数据库master，
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回只读数据集</returns>
        private SqlDataReader ExecuteDataReader(CommandType commandType, string commandText,
        ...
        public DataTable ExecutePageDataTable(string sql, string order, int pageSize, int pageIndex,
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回影响的行数</returns>
        private static int ExecuteNonQuery(SqlConnection connection, SqlTransaction transaction,
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回只读数据集</returns>
        private static SqlDataReader ExecuteDataReader(string connectionString, string commandText,
            params SqlParameter[] parms) {
        ...
        /// <param name="parms">查询参数</param>
        /// <returns>返回只读数据集</returns>
        private static SqlDataReader ExecuteDataReader(string connectionString, CommandType commandType,
        ...
        public static DataRow ExecuteDataRow(string connectionString, string commandText, params SqlParameter[] parms) {
            var dt = ExecuteDataTable(connectionString, CommandType.Text, commandText, parms);
        ...
        public static DataRow ExecuteDataRow(string connectionString, CommandType commandType, string commandText,
            params SqlParameter[] parms) {
            var dt = ExecuteDataTable(connectionString, commandType, commandText, parms);
            return dt.Rows.Count > 0 ? dt.Rows[0] : null;
        }
        ...
        public static DataTable ExecuteEmptyDataTable(string connectionString, string tableName) {
            return ExecuteDataSet(connectionString, CommandType.Text,
        ...
        public static void BatchUpdate(string connectionString, string sql, int batchSize = 1000, int interval = 1) {
            ...
            while (ExecuteScalar<int>(connectionString, existsSql) != 0) {
                ExecuteNonQuery(connectionString, CommandType.Text,
                    Regex.Replace(sql, "update", string.Format("update top ({0})", batchSize),
