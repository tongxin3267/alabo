...
namespace Alabo.App.Core.Reports.Domain.Repositories {
    ...
    public class AutoReportRepository : RepositoryMongo<Report, ObjectId>, IAutoReportRepository {
        ...
        public List<object> GetDayCountReport(CountReportInput countReportInput) {
            ...
            //日期验证
            if (countReportInput.Condition.StartTime.ToString().IndexOf("0001") == -1 || countReportInput.Condition.EndTime.ToString().IndexOf("0001") == -1) {
                ...
                tempSqlWhere = @" and (CONVERT(VARCHAR(100), CreateTime, 23) >='" + varStartTime + "' and CONVERT(VARCHAR(100), CreateTime, 23)<= '" + varEndTime + "') ";
            }

            var sqlCountByDay = $@" select  CONVERT(VARCHAR(100), CreateTime, 23) Day, count(" + countReportInput.Field + $@") as TotalNum,
                                cast( convert (decimal(18,2),100*cast(count(distinct isnull(" + countReportInput.Field + $@",0)) as float)/cast(((select  count(" + countReportInput.Field + $@") from " + tableName + $@") )as float) ) as varchar)+'%' as SaleRatio
                                from " + tableName +
        ...
        public List<AutoReport> GetDayCountReportByFiled(CountReportInput countReport) {
            string tableName = ""; string specialField = ""; DateTime startTime = DateTime.Now; DateTime endTime = DateTime.Now; string sqlWhere = "";
            ...
            string tempSqlStatusCount = string.Empty;
            ...
            if (!string.IsNullOrEmpty(specialField)) {
                ...
                foreach (Enum item in Enum.GetValues(typeEnum)) {
                    ...
                    tempSqlStatusCount = tempSqlStatusCount + $@" count(CASE WHEN " + specialField + " =" + key + " THEN " + specialField + " END) AS  " + itemName.ToString() + " , ";
            ...
            List<AutoReport> result = new List<AutoReport>();

            var typeEnumExt = specialField.GetTypeByName();
            if (FilterSqlScript(sqlCountByDay) != true) {
                using (var dr = dbContext.ExecuteDataReader(sqlCountByDay)) {
                    while (dr.Read()) {
                        List<AutoReportItem> listItem = new List<AutoReportItem>();
                        AutoReport output = new AutoReport();
            ...
            if (!string.IsNullOrEmpty(EnumName)) {
                ...
                if (typeEnum != null) {
                    foreach (Enum item in Enum.GetValues(typeEnum)) {
                        ...
                        var itemValue = item.ToString();
                        var key = System.Convert.ToInt16(item);
                        tempSqlStatusCount = tempSqlStatusCount + $@" count(CASE WHEN " + countReportInput.Field + " =" + key + " THEN " + countReportInput.Field + " END) AS  " + itemName.ToString() + " , ";
                        chartCols.Add(itemName);
                    }
                }
            }
        ...
        public List<object> GetDayCountReportTableByFiled(CountReportInput countReport) {
            string tableName = ""; string specialField = ""; DateTime startTime = DateTime.Now; DateTime endTime = DateTime.Now; string sqlWhere = "";
            //字段别名
            string otherColName = string.Empty;
            string tempSqlWhere = string.Empty;
            ...
            string tempSqlStatusCount = string.Empty;
            string tempSqlStatusRatioCount = string.Empty;
            if (!string.IsNullOrEmpty(specialField)) {
                foreach (Status item in Enum.GetValues(typeof(Status))) {
                    var itemName = item.ToString();
                    var itemValue = ((int)item);
                    //统计状态数量
                    tempSqlStatusCount = tempSqlStatusCount + $@" count(CASE WHEN " + specialField + " =" + itemValue + " THEN " + specialField + " END) AS  CountStatus" + itemName.ToString() + " , ";
                    //统计状态比率
                    tempSqlStatusRatioCount = tempSqlStatusRatioCount + $@" cast( convert (decimal(18,2),100*cast(count(CASE WHEN " + specialField + "=" + itemValue + " THEN Status END) as float)/cast(((count(1) ) )as float) ) as varchar)+'%' as CountStatusRatio" + itemName.ToString() + ",";
                }
            }
        ...
        private Tuple<string, List<Columns>> ExeclCountTableSql(CountReportInput reportInput, string EnumName) {
            List<Columns> listCol = new List<Columns>();
            ...
            //日期验证
            if (reportInput.Condition.StartTime.ToString().IndexOf("0001") == -1 || reportInput.Condition.EndTime.ToString().IndexOf("0001") == -1) {
            ...
            if (!string.IsNullOrEmpty(EnumName)) {
                ...
                foreach (Enum item in Enum.GetValues(typeEnum)) {
                    ...
                    col = new Columns();
                    //统计状态数量
                    tempSqlStatusCount = tempSqlStatusCount + $@" count(CASE WHEN " + reportInput.Field + " =" + key + " THEN " + reportInput.Field + " END) AS  " + itemValue.ToString() + " , ";
                    col.name = itemName;
                    col.type = itemValue;
                    listCol.Add(col);
                    ...
                    //统计状态比率
                    tempSqlStatusRatioCount = tempSqlStatusRatioCount + $@" cast( convert (decimal(18,2),100*cast(count(CASE WHEN " + reportInput.Field + "=" + key + " THEN " + reportInput.Field + " END) as float)/cast(((count(1) ) )as float) ) as varchar)+'%' as " + itemNameRatioValue + " , ";

                    listCol.Add(col);
                }
            }

            var sqlCountByDay = $@" select  CONVERT(VARCHAR(100), CreateTime, 23) date, count(1) as count," + tempSqlStatusCount + tempSqlStatusRatioCount +
                                @"cast( convert (decimal(18,2),100*cast(count(distinct isnull(id,0)) as float)/cast(((select  count(id) from " + tableName + $@") )as float) ) as varchar)+'%' as rate
                                from " + tableName +
                                $@" where 1 = 1 " + tempSqlWhere +
                                $@" group by CONVERT(VARCHAR(100), CreateTime, 23) ";
        ...
        public List<CountReportTable> GetCountReportTableWithField(CountReportInput countReport, string EnumName) {
            ...
            {
                 new CountReportTable{ Name = "报表数据", AutoReportChart = new CountReportItem{ Columns = chartCols, Rows = chartRows }},
            };
        ...
        public List<SumReportTable> GetSumReportTable(SumReportInput reportInput, string EnumName) {
            ...
            {
                 new SumReportTable{ Name = "报表数据", SumReportTableItem = new SumReportTableItems{ Columns = chartCols, Rows = chartRows }},
            };
        ...
        private Tuple<string, List<string>> GetSumReportQuerySql(SumReportInput sumReportInput) {
            string tempSqlField = string.Empty;
            var chartCols = new List<string>();
            chartCols.Add("日期");

            //实体字段 过滤
            foreach (var field in sumReportInput.Fields) {
                var tempDisplayName = TypeExtensions.GetFiledDisplayName(sumReportInput.EntityType, field);
            ...
            if (sumReportInput.Condition.StartTime.ToString().IndexOf("0001") == -1 || sumReportInput.Condition.EndTime.ToString().IndexOf("0001") == -1) {
                tempSqlWhere = " and  CreateTime between CONVERT(VARCHAR(100), '" + sumReportInput.Condition.StartTime + "', 23) and CONVERT(VARCHAR(100), '" + sumReportInput.Condition.EndTime + "', 23)";
            }
            ...
            var sqlBaseExec = $@" select CONVERT(VARCHAR(100), CreateTime, 23) as 日期
                                   " + tempSqlField + @"
                                    ...
                                    where 1=1 " + tempSqlWhere +
                                    $@"group by CONVERT(VARCHAR(100), CreateTime, 23)";
            if (FilterSqlScript(sqlBaseExec) == true) {
        ...
        private Tuple<string, List<SumColumns>> GetSumTableQuerySql(SumReportInput reportInput, string EnumName) {
            ...
            string tempSqlField = string.Empty;
            string tempSqlStatusCount = string.Empty;
            List<SumColumns> colList = new List<SumColumns>();
            SumColumns col = new SumColumns();
            col.name = "日期";
            col.type = "Date";
            colList.Add(col);
            ...
            if (!string.IsNullOrEmpty(EnumName)) {
                ...
                if (typeEnum != null) {
                    foreach (Enum item in Enum.GetValues(typeEnum)) {
                        ...
                        itemName = FilterSpecial(itemName);
                        var itemValue = item.ToString();
                        var itemKey = System.Convert.ToInt16(item);
                        col = new SumColumns();
                        //实体字段
                        foreach (var field in reportInput.Fields) {
                            var tempDisplayName = TypeExtensions.GetFiledDisplayName(reportInput.EntityType, field);
                            //统计 Sum状态数量
                            var TotalAmountName = tempDisplayName + itemName;
                            var TotalAmountValue = field + itemValue;
                            //待完成 已完成 等状态
                            tempSqlStatusCount = tempSqlStatusCount + $@" , SUM(CASE WHEN " + reportInput.Field + "=" + itemKey + " THEN " + field + " ELSE 0 END) AS " + TotalAmountValue;
            ...
            //日期验证
            if (reportInput.Condition.StartTime.ToString().IndexOf("0001") == -1 || reportInput.Condition.EndTime.ToString().IndexOf("0001") == -1) {
                tempSqlWhere = tempSqlWhere + "  and  CreateTime between CONVERT(VARCHAR(100), '" + reportInput.Condition.StartTime + "', 23) and CONVERT(VARCHAR(100), '" + reportInput.Condition.EndTime + "', 23)";
            }
        ...
        public decimal GetSingleData(SingleReportInput singleReportInput) {
            ...
            switch (singleReportInput.Style) {
                case ReportStyle.Count:
                    sql.Append($@"SELECT ISNULL(COUNT(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName);
                    break;

                case ReportStyle.Sum:
                    sql.Append($@"SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName);
                    break;

                case ReportStyle.Avg:
                    sql.Append($@"SELECT ISNULL(AVG(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName);
                    break;

                case ReportStyle.Min:
                    sql.Append($@"SELECT ISNULL(MIN(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName);
                    break;

                case ReportStyle.Max:
                    sql.Append($@"SELECT ISNULL(MAX(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName);
                    break;
                ...
                case ReportStyle.ChainRatioYesterday:
                    ...
                    sql.Append($@" SELECT ISNULL(T2." + singleReportInput.Field + " - T1." + singleReportInput.Field + ",0) AS " + singleReportInput.Field + " FROM ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where convert(varchar(10),CreateTime,120)=convert(varchar(10),'" + yesterday + "',120) )T1 left join ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where convert(varchar(10),CreateTime,120)=convert(varchar(10),'" + today + "',120) )T2 on 1=1");
                    break;
                ...
                case ReportStyle.ChainRatioLastWeek:
                    ...
                    var lastWeekEnd = DateTime.Now.AddDays(-day - 6).AddDays(day - 1).ToString("yyyy-MM-dd 23:59:59");//上周对应当天

                    sql.Append($@" SELECT ISNULL(T2." + singleReportInput.Field + "-T1." + singleReportInput.Field + ",0) AS " + singleReportInput.Field + " FROM ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where CreateTime between '" + lastWeekStart + "' and '" + lastWeekEnd + "' )T1 left join ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where CreateTime between '" + thisWeekStart + "' and '" + thisWeekNow + "'  )T2 on 1=1");
                    break;
                ...
                case ReportStyle.ChainRatioLastMonth:
                    ...
                    sql.Append($@" SELECT ISNULL(T2." + singleReportInput.Field + "-T1." + singleReportInput.Field + ",0) AS " + singleReportInput.Field + " FROM ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where CreateTime between '" + lastMonthStart + "' and '" + lastMonthEnd + "' )T1 left join ");
                    sql.Append($@" (SELECT ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where CreateTime between '" + thisMonthStart + "' and '" + thisMonthNow + "'  )T2 on 1=1");
                    break;

                case ReportStyle.ChainRatioLastQuarter:

                    sql.Append($@" SELECT ISNULL(T2." + singleReportInput.Field + "- T1." + singleReportInput.Field + ",0) AS " + singleReportInput.Field + " FROM ");
                    sql.Append($@"(select ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where  DATEPART(qq,CreateTime)= DATEPART(qq,GETDATE())-1)T1 left join ");
                    sql.Append($@"(select ISNULL(SUM(" + singleReportInput.Field + "),0) AS " + singleReportInput.Field + " FROM " + tableName + " where  DATEPART(qq,CreateTime)= DATEPART(qq,GETDATE()))T2 on 1=1 ");
                    break;
