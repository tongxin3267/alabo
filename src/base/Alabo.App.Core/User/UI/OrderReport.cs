...
namespace Alabo.App.Core.User.UI {
    ...
    /// 采购统计
    /// </summary>
    [ClassProperty(Name = "采购统计")]
    public class OrderReport : IAutoReport {
        ...
        public List<AutoReport> ResultList(object query, AutoBaseModel autoModel) {
            ...
            var sqlMemberCount = $@" SELECT COUNT(*) FROM ZKShop_OrderAction  ";
            var memberCount = dbContext.ExecuteScalar(sqlMemberCount);
            var sqlNewOrder = $@" SELECT COUNT(*) FROM ZKShop_Order WHERE CreateTime > '{DateTime.Now.Date.ToString("yyyyMMdd")}' AND OrderType = 2";
            ...
            {
                new AutoReport{ Name = "电商统计", Icon = "Report1.icon", AutoReprotData = new AutoReprotData{ Type = ReportDataType.Amount ,  List = dataList }},
