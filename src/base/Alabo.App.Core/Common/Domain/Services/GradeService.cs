...
namespace Alabo.App.Core.Common.Domain.Services {
    ...
    public class GradeService : ServiceBase, IGradeService {
        ...
        public IList<UserGradeConfig> GetUserGradeList() {
            var userGrades = Resolve<IAutoConfigService>().GetList<UserGradeConfig>(r => r.Status == Status.Normal);
            if (userGrades.Count == 0) {
                ...
                gradeConfig.SetDefault();
                userGrades = Resolve<IAutoConfigService>().GetList<UserGradeConfig>(r => r.Status == Status.Normal);
            }
        ...
        public List<BaseGradeConfig> GetGradeListByKey(string key) {
            var config = Resolve<IAutoConfigService>().GetConfig(key);
