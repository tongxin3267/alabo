                ...
                if (Configuration.TriggerType != TriggerType) //触发类型与配置的触发类型不一致，直接退出
{
        ...
        protected string BillLogger(User TriggerUser, User GainerUser, string accountName, decimal TriggerAmount,
            decimal DividendAmount, string orderSerial = "")
        {
            ...
            return Configuration.TemplateRule.LoggerTemplate.Replace("{OrderUserName}", TriggerUser.UserName)
                ...
                .Replace("{GainerUserName}", GainerUser.UserName).Replace("{GainerNickName}", GainerUser.Name)
                .Replace("{GainerRealName}", GainerUser.Name)
                .Replace("{OrderSerial}", orderSerial).Replace("{AccountName}", accountName)
                .Replace("{ShareUserAmount}", TriggerAmount.ToString())
                .Replace("{DividendAmount}", DividendAmount.ToString());
        }
