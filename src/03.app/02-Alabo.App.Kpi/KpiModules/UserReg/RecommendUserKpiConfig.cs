...
namespace Alabo.App.Kpis.KpiModules.UserReg
{
    public class RecommendUserKpiConfig : KpiBaseConfig
    {
        ...
        [Field(ControlsType = ControlsType.Json, PlaceHolder = "请按会员等级升级点值的权重来设置，原则上等级越高，比例越高", ListShow = false,
            EditShow = true, ExtensionJson = "UserKpiRateItems")]
        [JsonIgnore]
        [Display(Name = "团队极差")]
    ...
    /// </summary>
    [TaskModule("23B9A703-6B42-4915-816F-D1208CC0000B", "团队级差收益(按比例)", SortOrder = 999999,
        ConfigurationType = typeof(RecommendUserKpiConfig), IsSupportMultipleConfiguration = true,
        FenRunResultType = FenRunResultType.Price, IsSupportSetDistriRatio = false,
        Intro =
            "不同的等级之间提成比例会有所不同，比如总监A的提成比例为30%，经理B的提成比例为20%,业务员的提成比例为10%。那么经理可以拿到业务员的级差为10%，总监可以拿到经理的级差为10%，业务员的级差为20%",
