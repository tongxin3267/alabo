...
using ZKCloud.Datas.Queries.Trees;
using ZKCloud.Domains.Dtos;
using ZKCloud.Domains.Trees;
...
namespace ZKCloud.Webs.Controllers {
    ...
    public abstract class TreeControllerBase<TDto, TQuery, TParentId> : WebApiControllerBase
        ...
        /// 树型服务
        /// </summary>
        private readonly ITreeService<TQuery, TParentId> _service;

        /// <summary>
        /// 初始化树型控制器
        /// </summary>
        /// <param name="service">树型服务</param>
        protected TreeControllerBase(ITreeService<TQuery, TParentId> service) {
            _service = service;
        }
        ...
        public virtual async Task<IActionResult> Enable([FromBody] string ids) {
            ...
            var result = await _service.FindByIdsAsync<TDto>(ids);
            return Success(result);
        }
