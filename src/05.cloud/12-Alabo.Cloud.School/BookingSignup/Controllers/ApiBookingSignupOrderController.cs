...
namespace Alabo.Cloud.School.BookingSignup.Controllers
{
    ...
    {
        public ApiBookingSignupOrderController()
        {
        ...
        {
            if (model.Name.IsNullOrEmpty()) return ApiResult.Failure("姓名不能为空");

            if (model.Mobile.IsNullOrEmpty()) return ApiResult.Failure("手机号不能为空");
