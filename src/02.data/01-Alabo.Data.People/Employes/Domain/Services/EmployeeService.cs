    ...
    public class EmployeeService : ServiceBase<Employee, ObjectId>, IEmployeeService {
        ...
        public Tuple<ServiceResult, RoleOuput> Login(UserOutput userOutput) {
            ...
            if (employee.IsSuperAdmin) {
                ...
                List<ThemeOneMenu> list = new List<ThemeOneMenu>();
                var listCount = 0;
                ...
                for (int i = 0; i < menus.Count; i++) {
                    ...
                    if (itemIds.Contains(item.Id)) {
                        ...
                        for (int j = 0; j < item.Menus.Count; j++) {
                            ...
                            if (itemNodeIds.Contains(itemNode.Id)) {
                                ...
                                foreach (var itemNodes in itemNode.Menus) {
                                    var itemNodesIds = postRole.RoleIds.Where(s => itemNode.Menus.Select(ss => ss.Id).Contains(s));
        ...
        public string GetLoginToken(GetLoginToken loginToken) {
            ...
            var token = key.ToString().ToMd5HashString() + key3.ToMd5HashString().Substring(5, 15) + key2.ToStr().ToMd5HashString().Substring(2, 18) + key3.ToMd5HashString();
        ...
        public LoginInput LoginByToken(GetLoginToken loginByToken) {
            ...
            if (loginByToken.Timestamp.ConvertToLong() < minTimestamp || loginByToken.Timestamp.ConvertToLong() > maxTimestamp) {
