...
namespace Alabo.Data.People.Employes.Domain.Services {
    ...
    public class PostRoleService : ServiceBase<PostRole, ObjectId>, IPostRoleService {
        ...
        public List<PlatformRoleTreeOutput> RoleTrees() {
            ...
            // 所有的岗位都包含首页权限，否则不能访问
            adminThemeMenus = adminThemeMenus.Where(r => !r.Url.Contains("admin/index", StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        public ServiceResult Edit(PostRoleInput model) {
            ...
            // 添加首页权限Id
            var adminThemeMenus = GetAdminThemeMenus();
            var indexMenu = adminThemeMenus.FirstOrDefault(r => r.Url.Contains("admin/index", StringComparison.OrdinalIgnoreCase));
            if (indexMenu != null) {
                postRole.RoleIds.Add(indexMenu.Id);
            }
            // 消息页面
            indexMenu = adminThemeMenus.FirstOrDefault(r => r.Url.Contains("admin/message", StringComparison.OrdinalIgnoreCase));
            if (indexMenu != null) {
                postRole.RoleIds.Add(indexMenu.Id);
            }
