...
namespace Alabo.Data.People.Users.UI
{
    public class InterfaceReport : IAutoReport
    {
        ...
        public List<AutoReport> ResultList(object query, AutoBaseModel autoModel)
        {
            ...
            var sqlNewOrder =
                $@" SELECT COUNT(*) FROM Basic_Record WHERE CreateTime > '{DateTime.Now.Date.ToString("yyyyMMdd")}'";
            ...
            var dataList = new List<AutoReprotDataItem>
            {
                ...
                {
                    Color = "Green", FontColor = "White", Name = "新订单", Intro = newOrderCount.ToString(),
                    Value = newOrderCount.ToString().ToDecimal(), Icon = "aa1.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Lime", FontColor = "White", Name = "跳出率", Intro = newOrderCount.ToString(),
                    Value = newOrderCount.ToString().ToDecimal(), Icon = "aa2.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Orange", FontColor = "White", Name = "用户注册数", Intro = memberCount.ToString(),
                    Value = memberCount.ToString().ToDecimal(), Icon = "aa3.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Red", FontColor = "White", Name = "访客", Intro = memberCount.ToString(),
                    Value = memberCount.ToString().ToDecimal(), Icon = "aa4.ico"
                }
            };
