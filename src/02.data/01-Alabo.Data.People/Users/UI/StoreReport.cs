...
namespace Alabo.Data.People.Users.UI
{
    public class StoreReport : IAutoReport
    {
        public List<AutoReport> ResultList(object query, AutoBaseModel autoModel)
        {
            ...
            var sqlMemberCount = @" SELECT COUNT(*) FROM Shop_Store ";
            var memberCount = dbContext.ExecuteScalar(sqlMemberCount);
            var sqlNewOrder =
                $@" SELECT COUNT(*) FROM Shop_Order WHERE CreateTime > '{DateTime.Now.Date.ToString("yyyyMMdd")}' AND OrderType = 4";
            ...
            var dataList = new List<AutoReprotDataItem>
            {
                ...
                {
                    Color = "Green", FontColor = "White", Name = "新订单", Value = newOrderCount.ToString().ToDecimal(),
                    Icon = "aa1.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Lime", FontColor = "White", Name = "跳出率", Value = newOrderCount.ToString().ToDecimal(),
                    Icon = "aa2.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Orange", FontColor = "White", Name = "用户注册数", Value = memberCount.ToString().ToDecimal(),
                    Icon = "aa3.ico"
                },
                new AutoReprotDataItem
                {
                    Color = "Red", FontColor = "White", Name = "访客", Value = memberCount.ToString().ToDecimal(),
                    Icon = "aa4.ico"
                }
            };
