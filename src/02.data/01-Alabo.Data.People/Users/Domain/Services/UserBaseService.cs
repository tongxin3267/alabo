    ...
    public class UserBaseService : ServiceBase, IUserBaseService {
        ...
        public Tuple<ServiceResult, UserOutput> Login(LoginInput loginInput) {
            ...
            if (!config.IsAllowUserLogin) {
                return new Tuple<ServiceResult, UserOutput>(ServiceResult.FailedWithMessage("系统维护升级中，禁止会员登录，请稍后重试"), null);
            }
            ...
            if (!loginInput.OpenId.IsNullOrEmpty()) {
                ...
                if (!Resolve<IOpenService>().CheckVerifiyCode(loginInput.UserName, loginInput.VerifyCode.ConvertToLong())) {
        ...
        public Tuple<ServiceResult, UserOutput> Reg(RegInput regInput) {
            ...
            if (!config.IsAllowUserReg) {
                return new Tuple<ServiceResult, UserOutput>(ServiceResult.FailedWithMessage("系统维护升级中，禁止会员注册，请稍后重试"), null);
            }
            var userConfig = Resolve<IAutoConfigService>().GetValue<UserConfig>();
        ...
        public ServiceResult Register(Alabo.Users.Entities.User user) {
            ...
            if (!user.UserName.StartsWith("WX")) {
                ...
                if (string.IsNullOrWhiteSpace(user.Detail.Password)) {
            ...
            // 开启第一次设置时密码为空
            if (userConfig.TheFirstUserSetPayPassword) {
                user.Detail.PayPassword = string.Empty;
            }
        ...
        TypeLable:
            ...
            if (user.GradeId.IsGuidNullOrEmpty()) {
                ...
                if (defaultGrade == null) {
                    return ServiceResult.FailedWithMessage("默认等级不存在，请联系管理员添加默认等级!");
                }
