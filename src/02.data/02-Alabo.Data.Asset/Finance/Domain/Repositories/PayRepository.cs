...
namespace Alabo.App.Core.Finance.Domain.Repositories {
    ...
    public class PayRepository : RepositoryEfCore<Pay, long>, IPayRepository {
        ...
        public ServiceResult AfterPay(Pay pay, bool isPaySucess, Trade trade) {
            var sqlList = new List<string>();
            var sql = string.Empty;
            var result = ServiceResult.Success;
            ...
            if (isPaySucess) {
                // 更新支付账单状态
                sql =
                    $"UPDATE [dbo].[Finance_Pay] SET [Message] = '{pay.Message}',[PayType]={Convert.ToInt16(pay.PayType)}  ,[ResponseSerial] ='{pay.ResponseSerial}' ,[Status] =2 ,[ResponseTime] = '{DateTime.Now}' where id={pay.Id} and Status=1";
                sqlList.Add(sql);
                // 插入分润订单
                ...
                if (pay.Type == CheckoutType.Recharge) {
                    var Status = 6L;
                    sql =
                        $"update  Finance_Trade set Status={Status}  where id  = {trade.Id}";
                    sqlList.Add(sql);

                    var accountSql =
                        $"select Amount from Finance_Account where MoneyTypeId='{trade.MoneyTypeId}' and UserId={pay.UserId}";
                    var amount = RepositoryContext.ExecuteScalar(accountSql).ToDecimal();
                    sql =
                        $"update Finance_Account set amount=amount+{trade.Amount} where UserId={pay.UserId} and MoneyTypeId='{trade.MoneyTypeId}'";
                    sqlList.Add(sql);

                    sql =
                        $@"INSERT INTO [dbo].[Finance_Bill]([UserId],[OtherUserId],[Type],[Flow] ,[MoneyTypeId],[Amount],[AfterAmount],[Intro]
                              ,[CreateTime] ,[EntityId])  VALUES({pay.UserId},0,{(int)BillActionType.Recharge},{(int)AccountFlow.Income},'{trade.MoneyTypeId}',{trade.Amount},{amount + trade.Amount},'充值{trade.Amount}','{DateTime.Now}',{trade.Id})";
                    sqlList.Add(sql);

                    // 分润订单, UserId 有等于 OrderId的可能, 用 EntityId = {trade.Id}) AND TriggerType != 1 联合进行限制
                    sql = $" IF NOT EXISTS(SELECT * FROM Task_ShareOrder WHERE EntityId = {trade.Id}) AND TriggerType != 1 " +
                        "INSERT INTO [dbo].[Task_ShareOrder]([UserId] ,[Amount] ,[EntityId],[Parameters] ,[Status],[SystemStatus] , [TriggerType] ,[Summary] ,[CreateTime] ,[UpdateTime],[Extension],[ExecuteCount]) " +
                        $"VALUES ({pay.UserId} ,{pay.Amount},{trade.Id},'' ,{(int)ShareOrderStatus.Pending} ,{(int)ShareOrderSystemStatus.Pending} ,{(int)TriggerType.Recharge} ,'' ,'{DateTime.Now}' ,'{DateTime.Now}','',0)";
                    sqlList.Add(sql);
                }
            }
